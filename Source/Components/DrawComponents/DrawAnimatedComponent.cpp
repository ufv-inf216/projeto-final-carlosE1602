...
void DrawAnimatedComponent::Draw(SDL_Renderer *renderer) {
    ...
    int width = mSpriteSheetData[spriteIdx]->w ;
    int heigth = mSpriteSheetData[spriteIdx]->h;
...
//    SDL_RendererFlip flip = mOwner->GetRotation() == 0.0f ? SDL_FLIP_NONE : SDL_FLIP_HORIZONTAL;
    SDL_RenderCopyEx(renderer, mSpriteSheetSurface, mSpriteSheetData[spriteIdx], &mDrawRect, mOwner->GetRotation(), nullptr, SDL_FLIP_NONE);
}
